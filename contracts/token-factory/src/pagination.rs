...
mod tests {
    ...
    fn test_get_tokens_first_page() {
        ...
        let result = get_tokens_by_creator(&env, &creator, no_cursor(), Some(20)).unwrap();
    ...
    fn test_get_tokens_pagination() {
        let (env, creator) = setup_with_tokens(50);
        let page1 = get_tokens_by_creator(&env, &creator, no_cursor(), Some(20)).unwrap();
    ...
    fn test_get_tokens_max_limit_enforced() {
        ...
        let result = get_tokens_by_creator(&env, &creator, no_cursor(), Some(150)).unwrap();
    ...
    fn test_get_tokens_default_limit() {
        let (env, creator) = setup_with_tokens(50);
        let result = get_tokens_by_creator(&env, &creator, no_cursor(), None).unwrap();
        assert_eq!(result.tokens.len(), 20);
        assert!(result.has_more);
    }
    ...
    fn test_get_tokens_cursor_past_end() {
        ...
        let result = get_tokens_by_creator(&env, &creator, invalid_cursor, Some(20)).unwrap();
        assert_eq!(result.tokens.len(), 0);
        assert!(!result.has_more);
    }
    ...
    fn test_get_tokens_deterministic_ordering() {
        ...
        let result1 = get_tokens_by_creator(&env, &creator, no_cursor(), Some(10)).unwrap();
    ...
    fn test_get_creator_token_count() {
        ...
        assert_eq!(get_creator_token_count(&env, &creator), 42);
    }
    ...
    fn test_get_creator_token_count_zero() {
        let env = Env::default();
        let creator = Address::generate(&env);
        assert_eq!(get_creator_token_count(&env, &creator), 0);
    }
    ...
    fn test_multiple_creators_isolated() {
        ...
        let result1 = get_tokens_by_creator(&env, &creator1, no_cursor(), Some(20)).unwrap();
        assert_eq!(result1.tokens.len(), 10);
        let result2 = get_tokens_by_creator(&env, &creator2, no_cursor(), Some(20)).unwrap();
