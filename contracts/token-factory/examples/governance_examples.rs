...
pub fn example_standard_dao_setup(env: &Env, admin: &Address) {
    ...
    println!("DAO configured: {}% quorum, {}% approval", 
...
pub fn example_check_proposal(
    ...
    // Check quorum
    let quorum_met = governance::is_quorum_met(
        total_votes,
        total_eligible,
    ...
    // Check approval
    let approval_met = governance::is_approval_met(
        yes_votes,
        total_votes,
...
pub fn example_governance_with_timelock(
    ...
    let change_id = timelock::schedule_fee_update(
        env,
        admin,
        Some(new_base_fee),
        None
...
pub fn example_dao_vote_scenario(env: &Env) {
    ...
    assert!(governance::is_quorum_met(total_votes_a, total_eligible, config.quorum_percent));
    assert!(governance::is_approval_met(yes_votes_a, total_votes_a, config.approval_percent));
    ...
    assert!(governance::is_quorum_met(total_votes_b, total_eligible, config.quorum_percent));
    assert!(!governance::is_approval_met(yes_votes_b, total_votes_b, config.approval_percent));
    ...
    assert!(!governance::is_quorum_met(total_votes_c, total_eligible, config.quorum_percent));
    assert!(governance::is_approval_met(yes_votes_c, total_votes_c, config.approval_percent));
